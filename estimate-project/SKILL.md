...
description: "Use when creating, naming, renaming, or filling in the metadata table of a project estimate document under docs/estimates/ — covers date-prefixed filenames keyed to the Date of assessment field, the required AI Estimate metadata row (never a separate Estimate or Human Estimate row), and the required Scope/Assumptions/Breakdown document structure. Always trigger when the user asks to estimate a project, size up a task, scope out how long something will take, or write/draft/create an estimate document — even if the request doesn't mention filenames, metadata, or docs/estimates/ directly. For turning the estimate into a client-facing price or quotation, tracking pricing variables, or Zoho Books estimates, see the quote-tracking skill instead."
version: 1.0.0
---
...
Once the `AI Estimate` is written, turning it into a client-facing price or quotation, tracking pricing variables in `docs/QUOTES.md`, or creating/updating a Zoho Books estimate is covered by the **quote-tracking** skill, not this one.
...
5. **Pricing** (only when a client-facing quote is requested) — handled by the **quote-tracking** skill, not this one. Load it before writing a price or quotation.
...
| Pricing the estimate for a client, or a Zoho Books estimate | Load the **quote-tracking** skill |
...
- Writing a client-facing price or quotation directly in this skill instead of loading **quote-tracking** for the pricing formula and quotation style.
