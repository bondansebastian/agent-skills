...
| 💵 Writing Quotations | Turn an AI Estimate into a client-facing price using a fully-variablized pricing formula (buffer multiplier, man-day hours, base man-day rate, currency) tracked in docs/QUOTES.md, write plain-business-language client quotations saved as dated Markdown files under docs/quotations/ (asking for the project name first, with a content-based fallback), only producing a polished .docx on explicit request, and handle scoped, non-destructive Zoho Books estimates (service-type line items, no unrequested man-hour detail). Triggers whenever the user asks to price, quote, or update a Zoho Books estimate. | [writing-quotations/SKILL.md](writing-quotations/SKILL.md) | `npx skills add https://github.com/bondansebastian/agent-skills --skill writing-quotations` |
