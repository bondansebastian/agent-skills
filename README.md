...
| 💵 Quote Tracking | Turn an AI Estimate into a client-facing price using a fully-variablized pricing formula (buffer multiplier, man-day hours, base man-day rate, currency) tracked in docs/QUOTES.md, write plain-business-language client quotations, and handle scoped, non-destructive Zoho Books estimates (service-type line items, no unrequested man-hour detail). Triggers whenever the user asks to price, quote, or update a Zoho Books estimate. | [quote-tracking/SKILL.md](quote-tracking/SKILL.md) | `npx skills add https://github.com/bondansebastian/agent-skills --skill quote-tracking` |
