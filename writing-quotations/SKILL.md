...
A new section with no equivalent in the source estimate document. Has these two fixed subsections, in this order, each rendered only when it has meaningful content (see "Omit Empty Sections" below):
...
- **`3.2 Perlu Konfirmasi`** (Needs Confirmation) — an explicit list of open questions the client still needs to answer before or during the work, each indexed (`3.2.1`, `3.2.2`, ...). This replaces relying on chat history for open questions — they live in the document itself. When there are no open questions, omit `3.2` entirely rather than rendering an empty heading.
...
### Omit Empty Sections (applies to every section and sub-section)

**Only render a section or sub-section that has meaningful content.** If it would be empty — or hold only a placeholder such as "-", "Tidak ada", "N/A", "TBD", or a lone heading with nothing under it — leave it out of the quotation entirely: no heading, no index, no placeholder line. This applies to every section and sub-section, not just `3.2 Perlu Konfirmasi` — e.g. `3.1 Keputusan Teknis`, the whole of Section 3 when both of its subsections are empty, any `4.x` "Opsi Tambahan", or the final "Catatan Estimasi" when there are no exclusions or caveats to state.

- **Indices stay stable.** Omitting a section never renumbers the others: if `3.2` is omitted, `3.1` stays `3.1`; if `3.1` is omitted, `3.2` keeps its index. The next free index rule is unchanged, and a later-added section takes its own index (a gap is fine).
- **Fixed, named subsections keep their index and heading** even when they are the only one rendered under their section. The single sub-item rule below does not collapse them into the parent — `3.1 Keputusan Teknis` alone stays `3.1 Keputusan Teknis` under `3 Catatan`.
- **Re-evaluate on every edit.** When a previously empty section gains content, render it under its stable index; when a section loses all its content, remove it. Treat this like any other change in the confirmation table (Change: Added / Removed).
- **The docx follows the `.md`.** A `.docx` is generated from the same content, so omitted sections are omitted there too.
- Sections 1, `2`, and `4.1` are core content of every quotation and are expected to always have content; if one truly can't be filled, ask the user instead of shipping an empty or placeholder section.
...
**Output format**: write the quotation as a `.md` file by default. Only build a `.docx` version when the user explicitly asks for one, every time — read `references/docx.md` first. It covers when a docx may be built, how to generate it, removing the estimate references from it, and naming each generation as a new file with the current `Versi` timestamp instead of overwriting.
...
| Producing a Word version | Only when explicitly requested, every time (never auto-regenerate) — see `references/docx.md`: generate via the `anthropic-skills:docx` skill, remove all references to the estimate from the docx, and write it as a new file named with the current `Versi` timestamp |
| Structuring the quotation | Fixed 4-section structure (omit any section/sub-section with no meaningful content — no heading, no placeholder, indices of the rest unchanged) with a unique hierarchical index on every section/sub-section: `1` Lingkup Pekerjaan, `2` Deliverables (grouped `2.x` headings back-referencing the estimate's Breakdown items), `3` Catatan (`3.1` Keputusan Teknis, `3.2` Perlu Konfirmasi — no standalone Asumsi section), `4` Estimasi Waktu & Investasi (`4.1` combined time+price, `4.2+` Opsi Tambahan, final `4.x` Catatan Estimasi) |
...
- Generating a `.docx` with `pandoc` or other ad hoc tooling instead of invoking the `anthropic-skills:docx` skill.
...
- Leaving the estimate back-references (e.g. `estimasi dari 3.1 pada dokumen estimasi`) in the client-facing `.docx` — they must be removed from the docx (but kept in the `.md`); see `references/docx.md`.
...
- Collapsing `3.1 Keputusan Teknis` and `3.2 Perlu Konfirmasi` back into a single "Catatan"/"Asumsi" section instead of keeping them as separate fixed subsections (each rendered only when it has content).
- Rendering a section or sub-section with no meaningful content (e.g. an empty `3.2 Perlu Konfirmasi`, or one holding just "Tidak ada"/"-"), or renumbering the remaining sections after omitting one.
