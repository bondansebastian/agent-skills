...
| Producing a Word version | Only when explicitly requested, every time (never auto-regenerate) — generate the `.docx` from the same Markdown content by invoking the `anthropic-skills:docx` skill, don't hand-author a separate copy or use `pandoc`, and write it as a new file named with the current `Versi` timestamp rather than overwriting any earlier docx |
